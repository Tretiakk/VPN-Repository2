...
public class ConfigParser {
    ...
    static public void useEmbeddedUserAuth(VpnProfile np, String inlinedata) {
        String data = VpnProfile.getEmbeddedContent(inlinedata);
        String[] parts = data.split("\\r?\\n");
        if (parts.length >= 2) {
            OpenVPNService.opvnUsername = parts[0];
            OpenVPNService.opvnPassword = parts[1];
    ...
    public VpnProfile convertProfile() throws ConfigParseError, IOException {
        ...
        if (authuser != null) {
            ...
            if (authuser.size() > 1) {
                ...
                np.mUsername = null;
                useEmbeddedUserAuth(np, authuser.get(1));
            }
        }
