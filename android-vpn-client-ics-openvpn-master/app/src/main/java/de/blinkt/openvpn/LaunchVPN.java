...
public class LaunchVPN extends Activity {
    ...
    private void askForPW(final int type) {
        ...
        EditText usernameET = userpwlayout.findViewById(R.id.username);
        EditText passwordET = userpwlayout.findViewById(R.id.password);
        CheckBox save_password = userpwlayout.findViewById(R.id.save_password);
        CheckBox show_password = userpwlayout.findViewById(R.id.show_password);

        if (type == R.string.password) {
            usernameET.setText(mSelectedProfile.mUsername);
            passwordET.setText(mSelectedProfile.mPassword);
            save_password.setChecked(!TextUtils.isEmpty(mSelectedProfile.mPassword));

            show_password.setOnCheckedChangeListener((buttonView, isChecked) -> {
                if (isChecked) passwordET.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD);
                else passwordET.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
            });
        ...
        dialog.setPositiveButton(android.R.string.ok, (dialog1, which) -> {
            if (type == R.string.password) {
                mSelectedProfile.mUsername = usernameET.getText().toString();

                String password = passwordET.getText().toString();
                if (save_password.isChecked()) {
                    mSelectedProfile.mPassword = password;
                } else {
                    mSelectedProfile.mPassword = null;
                    mTransientAuthPW = password;
                }
            } else {
            ...
            bindService(
                    new Intent(LaunchVPN.this, OpenVPNStatusService.class),
                    mConnection,
                    Context.BIND_AUTO_CREATE);

          //  finish();
        });
        dialog.setNegativeButton(android.R.string.cancel, (dialog2, which) -> {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == START_VPN_PROFILE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (needpw == R.string.password && OpenVPNService.opvnUsername != null){
                    mSelectedProfile.mUsername = OpenVPNService.opvnUsername;
                    mSelectedProfile.mPassword = null;
                    mTransientAuthPW = OpenVPNService.opvnPassword;
    ...
    void launchVPN() {
        int correctlyMassage = mSelectedProfile.checkProfile(this);

        if (correctlyMassage != R.string.no_error_found) {
            showConfigErrorDialog(correctlyMassage);
            return;
        }
        ...
        if (loadTunModule) executeSUcmd("insmod /system/lib/modules/tun.ko");
